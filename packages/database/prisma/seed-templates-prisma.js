...
/**
 * TEMPLATES SEED USING PRISMA CLIENT
 *
 * This uses the Prisma client directly to ensure compatibility with the schema.
 */
...
async function main() {
  console.log('Seeding templates using Prisma client...\n');

  // Get the Helix Code tenant using raw SQL (to avoid schema mismatch issues)
  const tenants =
    await prisma.$queryRaw`SELECT id, name FROM tenants WHERE slug = 'helix-code' LIMIT 1`;

  if (!tenants || tenants.length === 0) {
    console.error('❌ Tenant not found. Please ensure helix-code tenant exists.');
    process.exit(1);
  }
  ...
  for (const template of templates) {
    try {
      ...
      // Use Prisma client upsert to create or update
      await prisma.template.upsert({
        where: {
          id: `tpl-${tenant.id}-${template.type.toLowerCase()}-${template.name.toLowerCase().replace(/\s+/g, '-').substring(0, 25)}`,
        },
        update: {
